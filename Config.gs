...
// ====== Script Properties (Project Settings > Script Propertiesで設定) ======
const SCRIPT_PROPS       = PropertiesService.getScriptProperties();
const API_KEY            = SCRIPT_PROPS.getProperty('API_KEY');
const WEBAPP_BASE_URL    = SCRIPT_PROPS.getProperty('WEBAPP_BASE_URL'); // ★★★ URLはこちらのプロパティ名を使用 ★★★
...
const KEY_MAP = {
  ...
  '改善後のリスクの見積もり': 'post_risk', '改善後の優先度': 'post_priority', '評価者による修正（最終）': 'modified_by_evaluator_final',
  'リスク低減値': 'risk_reduction_value', '最終評価コメント': 'final_eval_comment',
  '最終通知日時': 'final_notification_timestamp', '差し戻し理由': 'revert_reason'
};