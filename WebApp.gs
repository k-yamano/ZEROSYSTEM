...
function submitSecondaryEvaluation(data) {
  ...
  updateSheetRow_(id, { 'ステータス': '二次評価完了', '担当部署': department, '希望する評価者': hopeful_evaluator, '期限': deadline, '暫定予算': provisional_budget, '評価者によるコメント': secondary_eval_comment });
  const incidentData = getIncidentById(id);
  const webhookUrl = getDeptWebhookUrl_(department);
  if (webhookUrl) { sendToChat_(webhookUrl, `[${id}] 二次評価が入力されました。\n所属：${incidentData.department}\n確認担当部署：${department}\n${buildIncidentUrl(id)}`); }
  return `ID: ${id} の二次評価を受け付けました。`;
}
...
function submitImprovementReport(data) {
  ...
  const sheetUpdateData = {
      'ステータス': '改善報告済',
      '改善担当部署': incidentData.values['所属'], '改善完了報告': improvement_details,
...
function submitFinalEvaluation(data) {
  ...
  const reportUrl = buildIncidentReportUrl(id);
  updateSheetRow_(id, { 'ステータス': '最終評価完了', '最終評価コメント': final_eval_comment, 'レポート': reportUrl, '最終通知日時': new Date() });
...
function revertStatus(data) {
  ...
  try {
    ...
    if (targetStatus === '二次評価完了') {
      notifyEmail = incidentData.team_member_1; // 改善報告者へ
      message = `ID: ${id} の改善報告が差し戻されました。修正の上、再提出をお願いします。`;
    } else if (targetStatus === '改善報告済') {
      notifyEmail = incidentData.hopeful_evaluator; // 評価者へ
